        ...
        <div class="max-w-screen-md m-auto bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
            <div class="py-5">
                <div class="flex focus:outline-none">
                    <div class="m-5">
                            @if(is_null($authUser->userProfile->icon_image))
                                <img class="w-20 h-20 rounded" src="{{asset('images/no_image.png')}}">
                            @else
                                <img class="w-20 h-20 rounded" src="{{asset($authUser->userProfile->icon_image)}}">
                            @endif
                    </div>
                    <div class="my-5 font-semibold">{{ $authUser->userProfile->screen_name }}</div>
                </div>
                <form method="POST" action="{{route('tweet.update',['id' => $editingTweet->id])}}" enctype="multipart/form-data">
                    @csrf
                    <div class="my-5 ml-5">
                        <label for="content" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400"></label>
                        <input id="content" value="{{$editingTweet->content}}"  name="content" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></input>
                    </div>
                    <div class="flex justify-around">
                            @if(!is_null($editingTweet->image))
                                <img class="w-20 h-20 rounded" src="{{asset($editingTweet->image)}}">
                            @endif
                    </div>
                    <div class="mb-8 ml-5">
                    ...
                    <div class="mt-5 mx-5 text-s flex justify-end">{{ $editingTweet->updated_at }}</div>
                    <div class="flex justify-around">
